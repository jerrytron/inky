...
// This version of the story is functionally identical to 'another-day.ink'.
// The story structure instead splits into two 'knots', each holding an arbitrary set
// of 'stitches'. Normally the purpose of using a combination would be to organize
// your writing better, so use this example purely to better understand the syntax.
// Also remember, if you wanted you could put content and choices directly underneath
// the knots instead of just using them as headings.

=== start ===
// Instead of having the stitch 'wander', I could just have the 'wander' content and choices directly under 'start'.
...
= wander
    ...
    // Notice the different linking syntax if the destination is inside a different knot.
    + I want to PUNCH the art! -> middle.punch_the_art
...
= art_fierce
    ...
    // Since 'jurassic_park' is the first stitch in the 'middle' knot, and the knot doesn't have any of it's own content, it will default to the first stitch.
    + Jurassic Park. -> middle
    + Koyaanisqatsi. -> middle.koyaanisqatsi
...
= punch_the_art
    ...
    + Maybe the pain is GOOD. -> start.prove_it
